(() => {
  ...
  const DECISION_COLORS = {
    approved: "#2C3240", rejected: "#6E7A94", withdrawn: "#AEB6C8",
    pending: "#D7DBE3", other: "#F3F4F7"
  };
    ...
    try {
      if (window.grist.onOptions) {
        window.grist.onOptions((options) => {
          loadCalendarNotes(options?.calendarNotesV1);
        });
      }
      window.grist.ready({ requiredAccess: "read table", allowSelectBy: true, columns: COLUMN_DEFS });
      if (window.grist.widgetApi?.getOption) {
        window.grist.widgetApi.getOption("calendarNotesV1").then(loadCalendarNotes).catch(() => {});
      }
  ...
  function startStandalone() {
    try { loadCalendarNotes(JSON.parse(localStorage.getItem("pkmSpsCalendarNotesV1") || "[]")); } catch (_) { loadCalendarNotes([]); }
  ...
  function renderCalendar() {
    ...
    const days = Array.from({ length: 42 }, (_, i) => {
      ...
      const dataShown = records.slice(0, remainingSlots).map((record, index) => `<button type="button" class="calendar-event" data-date="${key}" data-event-index="${index}" title="${escapeAttr(record.PemajuResolved)}">${escapeHtml(record.PemajuResolved)}</button>`).join("");
      ...
      return `<div class="calendar-day${inMonth ? "" : " is-outside"}${today ? " is-today" : ""}" data-calendar-date="${key}" tabindex="0" role="button" aria-label="Tambah nota pada ${formatDate(date)}"><div class="calendar-date"><span>${date.getDate()}</span>${total ? `<b>${total}</b>` : ""}</div><div class="calendar-events">${manualShown}${dataShown}${more}</div></div>`;
    }).join("");
  ...
  async function persistCalendarNotes(notes) {
    try {
      if (state.mode === "grist" && window.grist?.widgetApi?.setOption) {
        await window.grist.widgetApi.setOption("calendarNotesV1", notes);
      } else {
        localStorage.setItem("pkmSpsCalendarNotesV1", JSON.stringify(notes));
      }
      state.calendarNotes = notes;
      return true;
    } catch (error) {
      showToast("Nota tidak dapat disimpan. Pastikan anda mempunyai kebenaran mengedit dokumen Grist.");
      return false;
    }
  }
